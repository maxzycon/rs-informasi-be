...
func (s *GlobalService) GetDashboardAnalytic(ctx context.Context) (resp *dto.SummaryDashboardWrapper, err error) {
	...
	for yearlyChart.Next() {
		...
		resp.QueuesChart = append(resp.QueuesChart, temp)
	}
	// ----- Location
	locationChart, err := s.db.
		Raw(`SELECT COUNT(q.id) as total, l.name 
				...
				WHERE YEAR(q.created_at) = YEAR(now()) AND MONTH(q.created_at) = MONTH(now())
	...
	for locationChart.Next() {
		...
		resp.QueuesLocationChart = append(resp.QueuesLocationChart, temp)
	}
	// ----- Type
	...
	for typeChart.Next() {
		...
		if temp.Type == typequeue.RACIKAN {
			resp.QueuesType.Racikan = temp.Value
		}

		if temp.Type == typequeue.NON_RACIKAN {
			resp.QueuesType.NonRacikan = temp.Value
		}
	}
	...
	for avgType.Next() {
		...
		if temp.Type == typequeue.RACIKAN {
			resp.QueuesAvgType.Racikan = temp.Value
		}

		if temp.Type == typequeue.NON_RACIKAN {
			resp.QueuesAvgType.NonRacikan = temp.Value
		}
	}

	// ----- avg by status
	err = s.db.Raw(`SELECT 
			AVG(COALESCE(TIMESTAMPDIFF(MINUTE, f.created_at, t.created_at), 0)) as validation_to_process,
			AVG(COALESCE(TIMESTAMPDIFF(MINUTE, t.created_at, th.created_at), 0)) as process_to_ready_to_be_submitted,
			AVG(COALESCE(TIMESTAMPDIFF(MINUTE, th.created_at, fr.created_at), 0)) as ready_to_be_submitted_to_submitted
			FROM queues q
			LEFT JOIN (SELECT queue_id, status FROM queue_histories as c WHERE id = (SELECT MAX(qh.id) FROM queue_histories as qh WHERE qh.queue_id = c.queue_id))  as l ON l.queue_id = q.id
		...
		Row().
		Scan(&resp.QueuesAvgStatus.ValidationToProcess, &resp.QueuesAvgStatus.ProcessToReadyToBeSubmitted, &resp.QueuesAvgStatus.ReadyToBeSubmitted)
